...
class RaBitQSpace : public SpaceInterface<float> {
 public:
    ...
    static constexpr float kCodeOffset = static_cast<float>(kBaseMax) * 0.5f;
 ...
 private:
    ...
    void setCompactCodeValue(uint8_t *code, size_t dim, uint8_t value) const {
        value = static_cast<uint8_t>(value & 0x0F);
        uint8_t &byte = code[dim >> 1U];
        if ((dim & 1U) == 0) {
    ...
    float centeredCodeValue(uint8_t value) const {
        return static_cast<float>(value) - kCodeOffset;
    }
    ...
    void exrabitqGlobalCode(
        ...
        thread_local std::vector<uint8_t> current_code;
        thread_local std::vector<uint8_t> best_code;
        thread_local std::vector<std::pair<double, size_t>> breakpoints;

        current_code.assign(code_dim_, 0);
        best_code.assign(code_dim_, 0);
        breakpoints.clear();
        breakpoints.reserve(code_dim_ * static_cast<size_t>(kBaseMax));
        ...
        for (size_t i = 0; i < code_dim_; ++i) {
            ...
            const uint8_t initial_code = o < 0.0 ? 7 : 8;
            current_code[i] = initial_code;
            const double centered = static_cast<double>(initial_code) - static_cast<double>(kCodeOffset);
            dot_y_o_prime += centered * o;
            norm_y_sqr += centered * centered;

            if (std::abs(o) <= eps) {
                continue;
            }
            for (int k = 0; k < static_cast<int>(kBaseMax); ++k) {
                const double boundary = static_cast<double>(k) + 0.5;
                const double t = (boundary - static_cast<double>(kCodeOffset)) / o;
        ...
        size_t event_pos = 0;
        while (event_pos < breakpoints.size()) {
            const double cur_t = breakpoints[event_pos].first;
            while (event_pos < breakpoints.size() && breakpoints[event_pos].first == cur_t) {
                const size_t dim = breakpoints[event_pos].second;
                const double o = static_cast<double>(rotated_unit_data[dim]);
                const uint8_t old_code = current_code[dim];
                uint8_t new_code = old_code;
                if (o > 0.0 && old_code < kBaseMax) {
                    new_code = static_cast<uint8_t>(old_code + 1);
                } else if (o < 0.0 && old_code > 0) {
                    new_code = static_cast<uint8_t>(old_code - 1);
                }

                if (new_code != old_code) {
                    const double old_centered = static_cast<double>(old_code) - static_cast<double>(kCodeOffset);
                    const double new_centered = static_cast<double>(new_code) - static_cast<double>(kCodeOffset);
                    dot_y_o_prime += (new_centered - old_centered) * o;
                    norm_y_sqr += new_centered * new_centered - old_centered * old_centered;
        ...
        for (size_t i = 0; i < code_dim_; ++i) {
            setCompactCodeValue(compact_code, i, best_code[i]);
            final_dot_y_o_prime +=
                (static_cast<double>(best_code[i]) - static_cast<double>(kCodeOffset)) *
                static_cast<double>(rotated_unit_data[i]);
        }
    ...
    float exrabitqUnsignedDot(const QueryContext &query, const void *encoded) const {
        const uint8_t *code = codeBytes(encoded);
#if defined(__AVX512F__) || defined(__AVX2__)
        return exrabitqUnsignedDotSimd(query.rotated.data(), code);
#else
        return exrabitqUnsignedDotScalar(query.rotated.data(), code);
#endif
    }

    float exrabitqUnsignedDotScalar(
        const float *query_rotated,
        const uint8_t *code) const {
        float result = 0.0f;
        for (size_t i = 0; i < code_dim_; ++i) {
            result += query_rotated[i] * static_cast<float>(compactCodeValue(code, i));
        }
        return result;
    }

#if defined(__AVX512F__) || defined(__AVX2__)
    float exrabitqUnsignedDotSimd(
        const float *query_rotated,
        const uint8_t *code) const {
        alignas(64) float code_buffer[32];
        size_t i = 0;
#if defined(__AVX512F__)
        __m512 sum0 = _mm512_setzero_ps();
        __m512 sum1 = _mm512_setzero_ps();
        for (; i + 32 <= code_dim_; i += 32) {
            unpackUnsignedCodes(code, i, 32, code_buffer);
            const __m512 q0 = _mm512_loadu_ps(query_rotated + i);
            const __m512 q1 = _mm512_loadu_ps(query_rotated + i + 16);
            const __m512 c0 = _mm512_load_ps(code_buffer);
            const __m512 c1 = _mm512_load_ps(code_buffer + 16);
        ...
        for (; i + 32 <= code_dim_; i += 32) {
            ...
            const __m256 c0 = _mm256_load_ps(code_buffer);
            const __m256 c1 = _mm256_load_ps(code_buffer + 8);
            const __m256 c2 = _mm256_load_ps(code_buffer + 16);
            const __m256 c3 = _mm256_load_ps(code_buffer + 24);
    ...
    void unpackUnsignedCodes(
        const uint8_t *code,
        size_t offset,
        size_t count,
        float *code_out) const {
        for (size_t j = 0; j < count; ++j) {
            const size_t dim = offset + j;
            code_out[j] = static_cast<float>(compactCodeValue(code, dim));
        }
    }
#endif
    ...
    float queryDistancePrepared(const QueryContext &query, const void *encoded) const {
        ...
        const float dot_centered =
            exrabitqUnsignedDot(query, encoded) - kCodeOffset * query.sum_q;
    ...
    void decodeApproximate(const void *encoded, std::vector<float> &decoded) const {
        ...
        for (size_t i = 0; i < code_dim_; ++i) {
            const float centered = centeredCodeValue(compactCodeValue(code, i));
 ...
 public:
    explicit RaBitQSpace(size_t dim, size_t centroid_count = 1, uint32_t random_seed = 100)
        : dim_(dim),
          ...
          compact_code_bytes_((code_dim_ + 1) / 2),
    ...
    void saveState(std::ostream &output) const {
        const std::string magic = "EXRBTQ08";
    ...
    void loadState(std::istream &input) {
        ...
        if (!input.good() || std::string(magic, sizeof(magic)) != "EXRBTQ08") {
